(() => {
    ...
    const SKINS = {
      classic: {
        sceneBg: 0x05080a, // 아주 어두운 밤색/네이비톤
        fog: 0x05080a,
        tableFelt: 0x0a3320, // 묵직하고 어두운 카지노 그린
        tableFeltDark: 0x051f12,
        wood: 0x1e120d, // 다크 마호가니
        woodDark: 0x140a05, // 테두리 가죽/다크우드
        dealerButton: 0xd4af37, // 리얼 골드
        spot: 0xffe6c2, // 따뜻한 텅스텐 조명
        rimLeft: 0x224466, // 어두운 시네마틱 블루 섀도우
        rimRight: 0x886644, // 따뜻한 오렌지 앰비언트
        roomFloor: 0x080a0c,
        roomWall: 0x050709,
        roomWood: 0x1a100c,
        roomBrass: 0x8a6a32,
        neon: 0x051525,
        neonEmissive: 0x0a2a4a
      },
      neon: {
        sceneBg: 0x041222,
        fog: 0x041222,
    ...
    function buildLights() {
      // 1. 앰비언트 라이트(전체 밝기)를 확 낮춰서 어두운 지하실 느낌 조성
      const hemi = new THREE.HemisphereLight(0x8eb5e5, 0x1d1612, 0.1);
      ctx.scene.add(hemi);
  
      // 2. 테이블 중앙으로 떨어지는 시네마틱 스포트라이트 (강도 대폭 상향)
      const topSpot = new THREE.SpotLight(0xffe6c2, 8.0, 50, 0.7, 0.9, 1.2);
      topSpot.position.set(0, 10, 0); // 각도를 조금 더 수직으로
      topSpot.castShadow = true;
      topSpot.shadow.mapSize.width = 1024; // 그림자 선명하게
      topSpot.shadow.mapSize.height = 1024;
      topSpot.shadow.bias = -0.0005; // 그림자 깨짐 방지
      topSpot.target.position.set(0, 1, 0);
      ctx.scene.add(topSpot);
      ctx.scene.add(topSpot.target);
  
      // 3. 측면 드라마틱 음영 라이트
      const rimLeft = new THREE.PointLight(0x224466, 2.5, 22);
      rimLeft.position.set(-8, 3, 4);
      ctx.scene.add(rimLeft);
  
      const rimRight = new THREE.PointLight(0x886644, 1.8, 22);
    ...
    function buildTable() {
      const baseMat = new THREE.MeshStandardMaterial({ color: COLOR.wood, roughness: 0.85, metalness: 0.15 });
      ...
      // 테두리 가죽 (은은한 광택과 묵직함)
      const rimMat = new THREE.MeshStandardMaterial({ color: COLOR.woodDark, roughness: 0.35, metalness: 0.25 });
      ...
      // 골드 링 (진짜 황동/금속 느낌)
      const rimGoldMat = new THREE.MeshStandardMaterial({ color: 0xd9bc7f, roughness: 0.2, metalness: 0.85 });
    ...
    function animate() {
      ...
      if (ctx.lights) {
        // 바뀐 조명 스케일에 맞춰서 애니메이션 조정
        ctx.lights.topSpot.intensity = 8.0 + Math.sin(ctx.time * 0.55) * 0.2;
        ctx.lights.barFill.intensity = 0.2 + Math.sin(ctx.time * 0.8 + 1.1) * 0.05;
      }
